...
function checkedRadio(radio, error) {
  if (radio == "Uomo") {
    error += `<input type="radio" id="scelta" name="scelta" value="Uomo" checked>
    ...
    error += `<input type="radio" id="scelta" name="scelta" value="Uomo">
        <label for="Uomo">Uomo</label><br>
...
function requestHandler(request, response) {
  ...
  if (percorsoFile == "./recuperaDati") {
    ...
    request.on("end", function () {
      ...
      const cognome = post.cognome;
      const sesso = post.scelta;
      const nascita = post.data_nascita;
      const eta = post.eta;
      const email = post.email;
      const checkbox = post.cibo;
      let error = ``;
